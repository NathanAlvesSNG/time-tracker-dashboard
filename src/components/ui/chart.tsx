          ...
          .map((item, index) => {
            ...
            return (
              <div
                key={item.dataKey}
                className={cn(
                  "[&>svg]:text-muted-foreground flex w-[10rem] flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5",
...
function ChartTooltipContentWithFormattedHour({
  active,
  payload,
  className,
  indicator = "dot",
          ...
          .map((item, index) => {
            ...
            const formattedValue =
              item.value !== undefined
                ? formatHoursToHHMM(item.value as number)
                : "N/A";
